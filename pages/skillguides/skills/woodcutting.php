<?php
function getSkillContent($skill) { return <<<HTML
<h2>$skill Skill Guide</h2>
<p>
    Woodcutting - The Basics<br>
    Introduction<br>
    <br>One of the simplest ways in which people make a living in RuneScape, is through the woodcutters trade. <br><br>The woodcutting skill can benefit you in many ways, many of which involves using your logs for Firemaking, or members can use them to fletch items.<br><br><img style="display: inline;display: block; margin: 0 auto 0 auto;" src="https://web.archive.org/web/20061102142813im_/http://www.runescape.com/img/kbase/skills/woodcutting/screenshots/woodcuting_main.jpg" alt="[image]"><br><br>To chop down a tree, just click on a tree and select chop down tree. If you have an axe in your inventory and sufficient woodcutting level, you will attempt to chop down the tree. Although woodcutting is not a member skill, some items that can be cut using this skill require member status.<br><br>The symbol below indicates the location of the rarer trees on your maps:<br><br><img style="display: inline;display: block; margin: 0 auto 0 auto;" src="https://web.archive.org/web/20061102142813im_/http://www.runescape.com/img/kbase/key_icons/rare_trees.gif" alt="[image]"><br><br><h4 style="display: inline;">Tools</h4><br><br>Woodcutters need to be familiar with the tools of their trade. Below is a table that shows the types of woodcutting axes available.  Axes can be obtained in either of the following ways:<br><br>
        
    <table style="text-align: center;" align="center">
    <tr class="row_a">
    <td style="width: 100px; border-style: none;"><br style="clear: both;"><b>Tool</b></td>
    <td style="width: 100px; border-style: none;"><br style="clear: both;"><b>Level required</b></td>
    <td style="width: 300px; border-style: none;"><br style="clear: both;"><b>Obtained From</b></td>
    </tr>
    <tr class="row_b">
    <td><canvas data-itemname="bronze_axe"></canvas>Bronze Axe</td>
    <td>1</td>
    <td style="text-align: left;"><ul style="margin-top: 0px; margin-bottom: 0px; "><br><li>Stuck in a tree stump in the chicken pen, south-east of the Lumbridge windmill.<br><li>Bought from Bob's axe shop in Lumbridge<br><li>You can make one using the Smithing skill.</ul></td>
    </tr>
    <tr class="row_a">
    <td><canvas data-itemname="iron_axe"></canvas>Iron Axe</td>
    <td>1</td>
    <td style="text-align: left;"><ul style="margin-top: 0px; margin-bottom: 0px; "><br><li>Bought from Bob's axe shop in Lumbridge<br><li>You can make one using the Smithing skill.</ul></td>
    </tr>
    <tr class="row_b">
    <td><canvas data-itemname="steel_axe"></canvas>Steel Axe</td>
    <td>6</td>
    <td style="text-align: left;"><ul style="margin-top: 0px; margin-bottom: 0px; "><br><li>Bought from Bob's axe shop in Lumbridge<br><li>You can make one using the Smithing skill.</ul></td>
    </tr>
    <tr class="row_a">
    <td><canvas data-itemname="black_axe"></canvas>Black Axe</td>
    <td>6</td>
    <td style="text-align: left;"><ul style="margin-top: 0px; margin-bottom: 0px; "><br><li>Dropped by Black Demon's and the King Black Dragon.<br><li>Bought from other players.</ul></td>
    </tr>
    <tr class="row_b">
    <td><canvas data-itemname="mithril_axe"></canvas>Mithril Axe</td>
    <td>21</td>
    <td style="text-align: left;"><ul style="margin-top: 0px; margin-bottom: 0px; "><br><li>You can make one using the Smithing skill.<br><li>Dropped by the Tree Spirit random event.</ul></td>
    </tr>
    <tr class="row_a">
    <td><canvas data-itemname="adamant_axe"></canvas>Adamant Axe</td>
    <td>31</td>
    <td style="text-align: left;"><ul style="margin-top: 0px; margin-bottom: 0px; "><br><li>Dropped by the Tree Spirit random event.<br><li>You can make one using the Smithing skill.</ul></td>
    </tr>
    <tr class="row_b">
    <td><canvas data-itemname="rune_axe"></canvas>Rune Axe</td>
    <td>41</td>
    <td style="text-align: left;"><ul style="margin-top: 0px; margin-bottom: 0px; "><br><li>Dropped by the Tree Spirit random event.<br><li>You can make one using the Smithing skill.</ul></td>
    </tr>
    </table>
</p>
HTML; }